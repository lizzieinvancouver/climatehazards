...
See \verb|climhazards_QuercusUpdatedModel.pdf| for more on that. 
...
\item Fagus simulations future -- calculate the mean C 1 Nov to 1 Fev for 41 N site (we expect it to be around 13 at 0; and a little less than 13 at reduced SD), and for the other sites -- we expect more days above 13 with higher SD in all sites. 
...
\subsection{14 novembre 2023: Prep for meeting at CEFE this week}


\begin{enumerate}
\item Need to review our simulation findings -- most notes are in 23 juin 2023. 
\item Are we happy with the \emph{Quercus} parameters? We now use \verb{cmaes fit subset2 rep2.species} which I think (CHECK) is the full updated parameter set (not just the leaf and flower parameters)? 
\item Check my notes on Pinus results below with Isabelle.
\item Review results with future sims -- agree we can ignore calendar issue (and just toss plots in supp)?
\item Decide on SD and mean for one run with both ... do it, and interpret it. Do we have any predictions?
\item Which figures? Question: What do we want to present? We have a lot ....
\item Start writing up section on PHENOFIT so we can agree on it.
\item Questions for colleagues file. (DONE)
\item Smaller items
\begin{enumerate}
\item We will skip changing WHC, right?
\item I should write up the METHODS before I go, including:
\begin{enumerate}
