...
\subsection{Montpellier November 2025}

\subsubsection{14 November}

{\bf Next steps are:} work on plots, work on ms files. (See git issues.)\\

Last day, I am working through some queries that I gave myself yesterday on a piece of paper. Here they are with some answers: 

\begin{itemize}
\item Why do my 2023 paper notes (\verb|2023Jun_MontpellierVisitnotes.pdf|) have an arrow from Temperature to Survival? Answer (according to me and my notes here): is that it is in the equations but it never matters, see notes from 7 June 2024 in this file.
\item Should we give up on thermal curve \href{https://github.com/lizzieinvancouver/climatehazards/issues/16}{idea}? Answer (according to me): we might as will stick with the one that Isabelle liked for the paper for now and see how it goes. So I updated the issue (linked above) with next steps.
\item Which curve should we use for a conceptual figure? Discussed with Isabelle: We should use an endo-dormancy break curve.
\item Does hardiness operate throughout the whole year? Discussed with Isabelle: Yes.
\end{itemize}

I also ... 
\begin{itemize}
\item Made a paper figure with some conceptual figure ideas that I will scan and add to repo (paper is dated 14 Nov 2025)
\item Confirmed that it looks like I have looked at frost between endodormancy break date and leafout date (when change in hardiness is most critical), see phenofitleafindexplots.R to really confirm.
\item Noticed that I refer to some fruit maturation curve plots as endodormancy (which is wrong) -- I figured out this is probably just because I was copying endodormancy break curve code, so I fixed my language in the results file. 
\end{itemize}

\subsubsection{13 November: Working on my own...}

Today I worked on trying to get the paper started. I made a new section in \verb|climhazards_outline.tex| which maybe is useful? But maybe better would be to just start the ms file! Today I also:

\begin{itemize}
\item Fixed the missing years from the Fagus and Pinus historical sims and confirmed I am using DuputieChuine parameter set for Quercus in historical sims. 
\item Made some new issues to get myself going and organized.
\item Closed out some issues. 
\end{itemize}

\subsection{Montpellier June 2025!}

\subsubsection{13 June: Wrapping up ...}

Not my most productive day! I made some notes on issue \# 14 which is where I think I should pick up and I had a back and forth on issue \# 16 with Victor and Isabelle. I just need to will myself to start on the paper -- organize figures, write results. Come on! (It has been very hot though. It peaked Wednesday and was hot yesterday, also yesterday Victor, Isabelle and I discussed a potential Roscoff Monod seminar for several hours). 
...
\item Why does Quercus have more historical climate years than other species? (Update in Nov 2025: I fixed this, see related issue that I closed.)
\end{itemize}
