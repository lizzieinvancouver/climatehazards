...
% Results come from 6-7 June 2024 but I rewrote them in November 2025 (on my own). 

\subsection{Results for \emph{Fagus}} Results depend on latitude; more variance is usually bad (reduces fitness benefits of warming on fruit maturation) except when there are issues with late endodormancy break and thus variance sometimes improves fitness by pushing temperatures into the range where accumulation is possible.
\begin{itemize}
\item Site 41 N: Low variance is better except at high warming where there are issues with late breaking, see Fig. \ref{fig:fagussdmean41}. (Not shown: low variance appears always better at latitude 47 N.)
\item Site 53 N: Increased variance is constantly worse, but increased warming leads to less frost, see figure \ref{fig:fagussdmean53}. Fruit maturation stalls with warming (even though flowering keeps advances) though it appears to have no or low impact on fitness, see \ref{fig:fagusfruitmat53}.
\end{itemize}

\begin{figure} 
 \begin{center}
\noindent \includegraphics[width=1\textwidth]{..//analyses/graphs/phenofit/sims/meansdsimallmetricsFSlat41.pdf}
  \caption{\emph{Fagus} across changing temperature and variance across fitness components at 41\degree N latitude.}
  \label{fig:fagussdmean41}
  \end{center}
\end{figure}

\begin{figure} 
 \begin{center}
\noindent \includegraphics[width=1\textwidth]{..//analyses/graphs/phenofit/sims/meansdsimallmetricsFSlat53.pdf}
  \caption{\emph{Fagus} across changing temperature and variance across fitness components at 53\degree N latitude.}
  \label{fig:fagussdmean53}
  \end{center}
\end{figure}

\begin{figure} 
 \begin{center}
\noindent \includegraphics[width=1\textwidth]{..//analyses/graphs/phenofit/sims/extras/fruitmat/sdmeanfruitmatFagusMeanTemp53}
  \caption{Top: \emph{Fagus} fruit maturation curves (same curve, always in light blue, black is the daily temperatures) across changing mean temperatures with changing variance at 53\degree N latitude. Darker blue horizontal line shows the mean estimated daily maturation (result of Wang \& Engel for the given temperatures). You can see declines in estimated maturation with increasing mean, which explains stalling (or getting later) FruitMat with increasing means. This is one of two curves in this process, but the only one likely to do anything interesting (as the other is sigmoid). Back in 2024 we said this stalling/later ripening happened with increasing means and variance (see also issue 13), but then Victor pointed out we don't really see that (see issue 18). Hmm... }  % fruitmatplotsmeanvar.R 
  \label{fig:fagusfruitmat53}
  \end{center}
\end{figure}

\subsection{Results for \emph{Quercus}} 
Basically, everything is high fitness, with no major differences due to anything.

\begin{itemize}
\item At site 41: Flowering is earlier for higher variance, but very similar and no earlier mat date. Dormancy break date is later, but does not seem to lead to later flowering, and also does not affect fitness. Basically all fine (lower fitness a TINY bit for lower warming x higher variance). See Figure \ref{fig:quersdmean41}.
\item At 53 N: higher variance, lower warming leads to some frost but not much. See Figure \ref{fig:quersdmean53}.
\end{itemize}

\begin{figure} 
 \begin{center}
\noindent \includegraphics[width=1\textwidth]{..//analyses/graphs/phenofit/sims/meansdsimallmetricsQRlat41.pdf}
  \caption{\emph{Quercus} across changing temperature and variance across fitness components at 41\degree N latitude.}
  \label{fig:quersdmean41}
  \end{center}
\end{figure}

\begin{figure} 
 \begin{center}
\noindent \includegraphics[width=1\textwidth]{..//analyses/graphs/phenofit/sims/meansdsimallmetricsQRlat53.pdf}
  \caption{\emph{Quercus} across changing temperature and variance across fitness components at 53\degree N latitude.}
  \label{fig:quersdmean53}
  \end{center}
\end{figure}


\subsection{Results for \emph{Pinus}} Variance and warming compound to lead to greater negative effects at 1 degree of warming (at higher warming it's unclear). Or, as we said on the 7th (7 June 2024), ``everything is bad.'' See Figure \ref{fig:pinussdmean53}.


\begin{figure} 
 \begin{center}
\noindent \includegraphics[width=1\textwidth]{..//analyses/graphs/phenofit/sims/meansdsimallmetricsPSlat53.pdf}
  \caption{\emph{Pinus} across changing temperature and variance across fitness components at 53\degree N latitude. Results at 41 N not shown as survival is 0.1 at all warming, so nothing to see except 0 fitness.}
  \label{fig:pinussdmean53}
  \end{center}
\end{figure}
...
\item Drought survival should not matter as we do not change the water in the soil (WHC) or the precip. (It should not matter in historical data either, according to Isabelle on 14 Nov.)
\end{itemize}
