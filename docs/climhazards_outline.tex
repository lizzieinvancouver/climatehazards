...
\section{Flailing around in November 2025}
\emph{For now, I will confine myself here to this section in this document ...}

Questions
\begin{itemize}
\item What are the impacts of increasing temperatures versus increasing variance in temperatures? And when is the interaction important (versus they can simply be predicted from knowing the effects of each one alone)?
\item Do historical drivers extend into the future projections?
\end{itemize}

Thinking on setup ... not sure this is helpful... 
\begin{itemize}
\item  Things that could shift in general
\begin{itemize}
\item Changes in frost risk 
\end{itemize}
\item Things that could shift with warming
\begin{itemize}
\item Accelerated development (leaves, fruit), leading to fruit maturation happening more often
\item Delayed development (leaves only?) due to too much heat
\end{itemize}
\item Things that could shift with SD
\begin{itemize}
\item More frost events? 
\end{itemize}
\end{itemize}

Methods
\begin{itemize}
\item We used historical climate data (YEARS, dataset) and applied shifts to the historical data to simulate increases in means and shifts in variance. 
\item 
\item Drought survival should not matter as we do not change the water in the soil (WHC) or the precip. 
\end{itemize}

Maybe we also want something like ... \verb|2023Jun_MontpellierVisitnotes.pdf|?

Results
\begin{itemize}
\item Historical trends
\begin{itemize}
\item This is explained in Section 1 (incl. Fig 1 caption) in climhazardsresults.pdf ... we have no better plots so far, but you can see also: \\ \verb|analyses/graphs/phenofit/historical/allspp_maindriverxy.pdf|
\end{itemize}
\item Effects of increasing mean temperatures (summarized from results file, see below) %emw26Jun2026 -- I think I mean see below ... changing throughout
\item Fagus 
\begin{itemize}
\item Fagus depends on latitude:  at low latitudes there is reduced CarbonSurvival (not enough cold means late dormancy) and thus FruitMaturationDate gets later. While at higher latitudes there is an increase in the FruitIndex as FruitMaturation is higher. 
\item Quercus -- not much effect, FruitMaturation gets a little later at low latitudes due to ... ??
\item Pinus is pretty similar across latitudes, it just gets hammered by low CarbonSurvival due to low chilling and thus later dormancy break date
\end{itemize}
\item Effects of SD ... (copied from results file, see above)
\begin{itemize}
...
\end{itemize}
\item Effects of increasing mean temperatures and shifts in variance (summarized from results file, see below)
\begin{itemize}
\item Fagus depends a little on latitude: at site 41 N low variance is better except at high warming where there are issues with late breaking, at 53 N: Increased variance is constantly worse, but increased warming leads to less frost. Fruit maturation stalls (even though flowering advances) though it appears to have no or low impact on fitness, but we did look closely at it -- see Fig 21 in the results file.  
\item \emph{Pinus}: Variance and warming compound to lead to greater negative effects. We made some endodormancy curves, see Fig 19 of results file. 
\item Quercus: Not much happens, esp. at lower latitudes. At 53 N higher variance, lower warming leads to some frost but not much.
\end{itemize}
\end{itemize}
